...
export function AppointmentManager({ view = 'full' }: AppointmentManagerProps) {
  ...
  const [selectedService, setSelectedService] = useState<MedicalService | null>(services[0] ?? null);
  ...
  useEffect(() => {
    const temp = doctors.find((item) => item.id === form.doctorId) ?? null;
    setSelectedDoctor(temp);
  }, [form.doctorId]);

  useEffect(() => {
    const temp = services.find((item) => item.id === form.serviceId) ?? null;
  ...
  if (statusFilter === 'Scheduled') {
    filteredAppointments = filterScheduled(filteredAppointments);
  }

  if (statusFilter === 'Completed') {
    filteredAppointments = filterCompleted(filteredAppointments);
  }

  if (statusFilter === 'Cancelled') {
    filteredAppointments = filterCancelled(filteredAppointments);
  }
    ...
    <section className="panel booking-panel">
      ...
      <form className="appointment-form" onSubmit={handleSubmit}>
        ...
        <label>
          ...
          <select
            value={form.doctorId}
            onChange={(event) => handleInputChange('doctorId', event.target.value)}
          >
            {doctors.map((item) => (
          ...
          <article className="appointment-card" key={item.id}>
            ...
            <div className="appointment-actions">
              ...
              {item.status === 'Cancelled' && (
                <button className="danger-button" onClick={() => deleteAppointment(item.id)}>
                  Remove
                </button>
              )}
            </div>
